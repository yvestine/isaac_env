...
class PI0RemoteTAVLAConfig(PI0RemoteConfig):
    ...
    # simulator runs at 30 Hz while the real data is about 10 Hz, so the
    # teacher advances one action every three environment steps.
    num_history_steps: int = 1
    history_step_interval: int = 1
    action_start_index: int = 1
